...
PROJECT_NAME=portal-documentacao
STACK_NAME=docs-ericof-com
...
KEYCLOAK_VERSION=$(shell cat devops/keycloak/version.txt)
...
.PHONY: backend-start
backend-start: keycloak-start ## Start Plone Backend
...
keycloak-start:  ## Local Stack: Create a new site
	@echo "Start keycloak used in development mode"
	KEYCLOAK_VERSION=$(KEYCLOAK_VERSION) VOLTO_VERSION=$(VOLTO_VERSION) PLONE_VERSION=$(PLONE_VERSION) docker compose -f $(COMPOSE_FILE) up -d traefik keycloak --build
	@echo "Keycloak available at http://sso.localhost/"

.PHONY: keycloak-stop
keycloak-stop:  ## Local Stack: Create a new site
	@echo "Stop keycloak used in development mode"
	KEYCLOAK_VERSION=$(KEYCLOAK_VERSION) VOLTO_VERSION=$(VOLTO_VERSION) PLONE_VERSION=$(PLONE_VERSION) docker compose -f $(COMPOSE_FILE) down traefik keycloak db_keycloak

## Docker stack
.PHONY: stack-start
stack-start:  ## Local Stack: Start Services
	@echo "Start local Docker stack"
	KEYCLOAK_VERSION=$(KEYCLOAK_VERSION) VOLTO_VERSION=$(VOLTO_VERSION) PLONE_VERSION=$(PLONE_VERSION) docker compose -f $(COMPOSE_FILE) up -d --build
	@echo "Now visit: http://docs.localhost"

.PHONY: stack-create-site
stack-create-site:  ## Local Stack: Create a new site
	@echo "Create a new site in the local Docker stack"
	@docker compose -f $(COMPOSE_FILE) exec backend ./docker-entrypoint.sh create-site
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ericof/portal-documentacao-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ericof/portal-documentacao-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name portal-documentacao-frontend-acceptance --link portal-documentacao-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ericof/portal-documentacao-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name portal-documentacao-backend-acceptance -d ericof/portal-documentacao-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop portal-documentacao-frontend-acceptance
	@docker stop portal-documentacao-backend-acceptance
